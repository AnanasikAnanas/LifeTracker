...
- Есть учет финансов: доходы, расходы, категории, счета, баланс дня.
- Есть голосовой ввод в форме добавления. Фраза вроде `расход кофе 420 еда карта` заполнит финансовую операцию.
...
В `package.json` build-команда запускает TypeScript и Vite через `node`:

```json
"build": "node ./node_modules/typescript/lib/tsc.js && node ./node_modules/vite/bin/vite.js build"
```

Это обход проблемы `Permission denied` на `node_modules/.bin/tsc` в Linux-сборке Vercel. После изменения команды сделайте новый deploy. Если Vercel использует старый cache, нажмите `Redeploy` -> `Use existing Build Cache` выключить.

## Голосовой ввод

Голосовой ввод работает через Web Speech API. На HTTPS в поддерживаемых браузерах появится запрос доступа к микрофону. В некоторых Telegram WebView распознавание речи может быть недоступно; тогда используйте поле `Распознанный текст` вручную, оно применяет ту же логику автозаполнения.
